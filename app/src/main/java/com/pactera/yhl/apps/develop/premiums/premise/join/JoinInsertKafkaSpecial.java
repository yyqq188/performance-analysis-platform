...
public class JoinInsertKafkaSpecial<OUT> extends RichSinkFunction<OUT> {
    ...
    public JoinInsertKafkaSpecial(String tableName, String topic,
...
//                            .format(new Date(System.currentTimeMillis()));
                ...
                }else if(f.getName().toString().equals("signdate")){
                    if(f.get(value).toString().startsWith("YBT")){
                        String methodName = "set"+Util.LargerFirstChar(f.getName());
                        Method method = kafkaClazz.getDeclaredMethod(methodName, String.class);
                        method.invoke(kafkaClazzObj,Util.toString(f.get(value)));
                        driverFilterNum += 1;
