 ...
 * company: gientech
 */
public class ProductLCFlatMap extends RichFlatMapFunction<LbpolKafka05,LbpolKafka05> {
    ...
    public void flatMap(LbpolKafka05 lbpolKafka05, Collector<LbpolKafka05> collector) throws Exception {
        double prem = Double.valueOf(lbpolKafka05.getPrem());
        if(lbpolKafka05.getRate().equals("")
                ||lbpolKafka05.getRate().length() == 0
                || Objects.isNull(lbpolKafka05.getRate())){
            reducingState.add(prem);
        }else {
            Double rate = Double.valueOf(lbpolKafka05.getRate());
