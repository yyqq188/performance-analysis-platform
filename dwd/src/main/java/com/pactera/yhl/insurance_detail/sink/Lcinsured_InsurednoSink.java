 ...
 */
public class Lcinsured_InsurednoSink extends AbstractInsertHbase<KLEntity> {
    public Lcinsured_InsurednoSink() {
        tableName = "KL:LCINSURED_INSUREDNO";//HBase中间表名
        rowkeys = new String[]{"insuredno"};//HBase rowkeys
        //POJO主键---HBase列名
        columnNames = new String[]{"contno", "insuredno"};
        columnTableName = "lcinsured";//表名
    }

    @Override
    public void handle(KLEntity klEntity, Context context, HTable table) throws Exception {
        Lcinsured value = (Lcinsured) klEntity;
