 ...
 */
public class Lbinsured_InsurednoSink extends AbstractInsertHbase<KLEntity>{
    public Lbinsured_InsurednoSink(){
        tableName = "KL:LBINSURED_INSUREDNO";//HBase中间表名
        rowkeys = new String[]{"insuredno"};//HBase rowkeys
        //POJO主键---HBase列名
        columnNames = new String[]{"contno" ,"insuredno"};
        columnTableName = "lbinsured";//表名
    }

    @Override
    public void handle(KLEntity klEntity, Context context, HTable table) throws Exception {
        Lbinsured value = (Lbinsured) klEntity;
