 ...
 */
public class Kl_lpedoritem_EdornoSink extends AbstractInsertHbase<KLEntity> {
    public Kl_lpedoritem_EdornoSink() {
        tableName = "KL:KL_LPEDORITEM_EDORNO";//HBase中间表名
        rowkeys = new String[]{"edorno"};//HBase rowkeys
        //POJO主键---HBase列名
        columnNames = new String[]{"contno"};
        columnTableName = "kl_lpedoritem";//表名
    }

    @Override
    public void handle(KLEntity klEntity, Context context, HTable table) throws Exception {
        KL_lpedoritem value = (KL_lpedoritem) klEntity;
