...
/**
 * @author Sun Haitian
 * @Description 接收 ods.o_lis_lccontextend lct, 插入luc表数据, 取 自护保标识+消费方式
 * @create 2021/10/14 10:34
 */
public class FP1Joinlct_luc extends AbstractJoin<Lccontextend, KL_lucont>{
    //中间表对象
...
//    left join ods.o_lis_lccontextend lct --自护保标识+消费方式
//    on luc.contno = lct.contno
//    primarykey : contno
    @Override
    public void asyncHandler(Lccontextend lct) throws Exception {
        //获取lct主键及关联字段
        String contno = lct.getContno();
        //插入宽表信息
        String lctValue = JSON.toJSONString(lct, SerializerFeature.WriteMapNullValue,
            ...
            for (Cell lucListCell : lucResult.listCells()){
                ...
                if (!lupResult.isEmpty()){
                    for(Cell lupListCell : lupResult.listCells()){
                        ...
                        put.addColumn(cf,Bytes.toBytes("fp1_lccontextend" + contno),
                                Bytes.toBytes(lctValue));
                        kuanbiao.put(put);
                    }
                }
            }
