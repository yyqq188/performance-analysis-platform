 ...
 */
public class Lbappnt_AppntNo_AddressNoSink extends AbstractInsertHbase<KLEntity>{

    public Lbappnt_AppntNo_AddressNoSink(){
        tableName = "KL:LBAPPNT_APPNTNO_ADDRESSNO";//HBase中间表名
        rowkeys = new String[]{"appntno","addressno"};//HBase rowkeys
        //POJO主键---HBase列名
        columnNames = new String[]{"contno"};
        columnTableName = "lbappnt";//表名
    }

    @Override
    public void handle(KLEntity klEntity, Context context, HTable table) throws Exception {
        Lbappnt value = (Lbappnt) klEntity;
