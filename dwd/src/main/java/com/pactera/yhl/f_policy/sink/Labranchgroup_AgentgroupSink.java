...
import com.pactera.yhl.entity.Labranchgroup;
 ...
 * @description:
 * @author: TSY
 * @create: 2021/10/12 0012 下午 15:36
 * @Desc:
 */
public class Labranchgroup_AgentgroupSink extends AbstractInsertHbase<KLEntity>  {
    public Labranchgroup_AgentgroupSink(){
        tableName = "KL:LABRANCHGROUP_AGENTGROUP";//HBase中间表名
        rowkeys = new String[]{"agentgroup"};//HBase rowkeys
        //POJO主键---HBase列名
        columnNames = new String[]{"agentgroup"};
        columnTableName = "labranchgroup";//表名
    }
    @Override
    public void handle(KLEntity klEntity, Context context, HTable table) throws Exception {
        Labranchgroup value = (Labranchgroup)  klEntity;
