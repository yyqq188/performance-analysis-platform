...
import com.pactera.yhl.entity.Laagent;
 ...
 * @description:
 * @author: TSY
 * @create: 2021/10/12 0012 下午 15:24
 * @Desc:
 */
public class Laagent_AgentCodeSink extends AbstractInsertHbase<KLEntity>  {
    public Laagent_AgentCodeSink(){
        tableName = "KL:LAAGENT_AGENTCODE";//HBase中间表名
        rowkeys = new String[]{"agentcode"};//HBase rowkeys
        //POJO主键---HBase列名
        columnNames = new String[]{"agentcode"};
        columnTableName = "laagent";//表名
    }
    @Override
    public void handle(KLEntity klEntity, Context context, HTable table) throws Exception {
        Laagent value = (Laagent)  klEntity;
