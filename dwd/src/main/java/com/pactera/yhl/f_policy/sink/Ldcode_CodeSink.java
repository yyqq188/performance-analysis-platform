 ...
 * @description:
 * @author: TSY
 * @create: 2021/10/12 0012 下午 15:47
 * @Desc:
 */
public class Ldcode_CodeSink extends AbstractInsertHbase<KLEntity>  {
    public Ldcode_CodeSink(){
        tableName = "KL:LDCODE_CODE";//HBase中间表名
        rowkeys = new String[]{"code"};//HBase rowkeys
        //POJO主键---HBase列名
        columnNames = new String[]{"codetype", "code"};
        columnTableName = "ldcode";//表名
    }
    @Override
    public void handle(KLEntity klEntity, Context context, HTable table) throws Exception {
        Ldcode value = (Ldcode)  klEntity;
