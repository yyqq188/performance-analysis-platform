 ...
 * @description:
 * @author: TSY
 * @create: 2021/10/15 0015 下午 14:59
 * @Desc:
 *
 * left join KL_LPEDORITEM lp
 * on lp.EDORNO=lup.EDORNO and lp.EDORTYPE='WT' AND  lp.EDORSTATE='0'--限定状态为犹退且保全状态完成1009 修改
 * and lp.contno=luc.contno
 */
public class FP3Lp_LucLup extends AbstractJoin<KL_lpedoritem, KL_lupol>{
    //中间表对象
    ...
    public void asyncHandler(KL_lpedoritem lp) throws Exception {
        ...
        if (!lupResult.isEmpty()) {
            for (Cell listCell : lupResult.listCells()) {
                ...
                String lupEdortype = jsonObject.getString("edortype");
                String lupEdorstate = jsonObject.getString("edorstate");
                //判断lup.source_id='1'
                if ("WT".equals(lupEdortype) && "0".equals(lupEdorstate)) {
                    ...
                    if (!lucResult.isEmpty()) {
                        for (Cell cell : lucResult.listCells()) {
                            ...
                            put.addColumn(cf, Bytes.toBytes("fp3_kl_lpedoritem" + contno + edoracceptno + edorno + edortype + insuredno + polno), Bytes.toBytes(lpValue));
                            kuanbiao.put(put);
                        }
                    }
                }
